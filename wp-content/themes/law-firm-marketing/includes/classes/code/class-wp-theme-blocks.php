...
namespace LawFirmMarketing\Blocks;
...
class WP_Theme_Blocks {
	...
	public function register_acf_blocks() {
		...
		register_block_type( LAWFIRMMARKETING_BLOCK_DIR . '/section-container' );
	...
	protected static function register_acf_block( $block_name = null, $has_script = false, $block_scripts = null ) {
		if ( $has_script ) {
			...
			$dependencies       = array();
			if ( $block_scripts ) {
				$dependencies = LawFirmMarketing::register_scripts( $block_scripts );
			}
			$block_script_order = array_merge( $block_script_order, $dependencies );
			LawFirmMarketing::register_script(
		...
		register_block_type( LAWFIRMMARKETING_BLOCK_DIR . '/' . $block_name );
	}
}
new WP_Theme_Blocks();
