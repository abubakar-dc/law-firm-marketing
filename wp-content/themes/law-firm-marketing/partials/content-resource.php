...
$lmf_var_post_title = $lmf_fields['lmf_var_post_title'] ?? get_the_title();
...
<div class="container-980">
	<div class="wrapper">
		<div class="post-image">
			...
			<?php
			if ( ! has_post_thumbnail( $lmf_var_post_id ) ) {
		...
		<div class="post-meta d-flex justify-content-between align-items-center">
			<!-- /.post-tags -->
			<?php if ( $lmf_var_post_categories ) { ?>
				<div class="post-cat">
					<?php foreach ( $lmf_var_post_categories as $lmf_var_category ) { ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-ctn' ); ?>>
			...
			$lmf_var_rp_selection_criteria = isset( $lmf_fields['lmf_var_rp_selection_criteria'] ) ? $lmf_fields['lmf_var_rp_selection_criteria'] : null;
			if ( 'random' === $lmf_var_rp_selection_criteria ) {
				...
				global $post;
				$lmf_var_selected_posts = array();
				$lmf_var_selected_posts = isset( $lmf_fields['lmf_var_rp_selected_posts'] ) ? $lmf_fields['lmf_var_rp_selected_posts'] : null;
				if ( $lmf_var_selected_posts ) {

					?>
				<div class="related-posts ">
				<h3><?php esc_html_e( 'Related Posts', 'lawfirmmarketing_td' ); ?></h3>
							<?php
							foreach ( $lmf_var_selected_posts as $lmf_var_post_id ) {

								$lmf_post_fields = get_fields( $lmf_var_post_id );
								if ( ! has_post_thumbnail( $lmf_var_post_id ) ) {
									echo '<img class="" src="' . esc_url( get_template_directory_uri() ) . '/assets/build/images/admin/defaults/default-image.webp" >';
								} else {
									echo get_the_post_thumbnail(
