...
namespace LawFirmMarketing\Boilerplate;

use LawFirmMarketing;
/**
...
class WP_Theme_Boilerplate {
	...
	public static function block( $block, $callback_function ) {
		list( $lmf_block_id, $lmf_block_fields,$lmf_option_fields ) = LawFirmMarketing::defaults( $block['id'] );

		// Set the block name for it's ID & class from it's file name.
		$lmf_block_name   = $block['name'];
		$lmf_block_name   = str_replace( 'acf/', '', $lmf_block_name );
		$lmf_block_styles = LawFirmMarketing::convert_to_css( $block );
		// Set the preview thumbnail for this block for gutenberg editor view.
		if ( isset( $block['data']['preview'] ) && 'preview' === $block['mode'] && isset( $block['data']['preview'] ) ) {
			$version = filemtime( get_template_directory() . '/blocks/' . $lmf_block_name . '/' . $block['data']['preview'] );
			echo '<img src="' . esc_url( get_template_directory_uri() . '/blocks/' . $lmf_block_name . '/' . $block['data']['preview'] ) . '?v=' . esc_html( $version ) . '" style="width:100%; height:auto;">';
		}

		// create align class ("alignwide") from block setting ("wide").
		$lmf_var_align_class = $block['align'] ? 'align' . $block['align'] : '';

		// Get the class name for the block to be used for it.
		$lmf_var_class_name = ( isset( $block['className'] ) ) ? $block['className'] : null;

		// Making the unique ID for the block.
		$lmf_block_html_id = 'block-' . $lmf_block_name . '-' . $block['id'];

		// Making the unique ID for the block.
		if ( $block['name'] ) {
			$lmf_block_name = $block['name'];
			$lmf_block_name = str_replace( '/', '-', $lmf_block_name );
			$lmf_var_name   = 'block-' . $lmf_block_name;
		}
		echo '<div id="' . esc_html( $lmf_block_html_id ) . '" class="' . esc_html( $lmf_var_align_class . ' ' . $lmf_var_class_name . ' ' . $lmf_var_name ) . ' block-' . esc_html( $lmf_block_name ) . '" style="' . esc_html( $lmf_block_styles ) . '"> ';

		$spacers = self::get_spacer( $lmf_block_fields );
		LawFirmMarketing::the_spacer( $spacers, 'top' );
		$callback_function( $lmf_block_id, $lmf_block_name, $lmf_block_fields, $lmf_option_fields );
		LawFirmMarketing::the_spacer( $spacers, 'bottom' );
		echo '</div>';
		wp_enqueue_script( 'wp-theme-' . str_replace( 'acf-', '', $lmf_block_name ) );
	}
	/**
	 * Query Boilerplate.
	 *
		...
		if ( $post_type ) {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$lmf_var_post_id = get_the_ID();

					list( $lmf_var_post_id, $lmf_fields, $lmf_option_fields ) = LawFirmMarketing::defaults( $lmf_var_post_id );
					// Include specific template for the content.
					if ( isset( $template ) ) {
						get_template_part( 'partials/content', $template );
					} else {
						$render_fun( $lmf_var_post_id, $lmf_fields, $lmf_option_fields );
					}
				}
			} else {
				// If no content, include the "No posts found" template.
			...
			if ( $query->have_posts() && $query->max_num_pages > 1 && $pagination ) {
				...
				LawFirmMarketing::$pagination_fun( $query->max_num_pages );
				echo '</div>';
			}
			return $query;
		}
	 ...
	 * Helper Function
	 *
	 * @param mixed $lmf_block_fields block fields data.
	 *
	 * @return array
	 */
	private static function get_spacer( $lmf_block_fields ) {
		if ( $lmf_block_fields ) {
			foreach ( $lmf_block_fields as $lmf_block_field ) {
				if ( is_array( $lmf_block_field ) ) {
					foreach ( $lmf_block_field as $lmf_key => $lmf_block_sub_field ) {
						if ( 'top_spacer' === $lmf_key || 'bottom_spacer' === $lmf_key ) {
							return $lmf_block_field;
						}
					}
				}
			}
...
class_alias( 'LawFirmMarketing\Boilerplate\WP_Theme_Boilerplate', 'Boilerplate' );
