...
$lmf_var_elevate_ids = $attributes['elevateIds'] ?? null;
$lmf_var_stay_ids = $attributes['stayIds'] ?? null;
$lmf_var_post_values = array_merge($lmf_var_elevate_ids, $lmf_var_stay_ids);
$selectionMode = $attributes['selectionMode'] ?? 'recent';
$heading = $attributes['heading'] ?? '';
$description = $attributes['description'] ?? '';
$linkData = $attributes['linkData'] ?? null;

if ($lmf_var_post_values) {
?>
		...
		<div class="perk-slider-main">
			<div class="swiper perk-slider">
				<div class="swiper-wrapper">
					<?php
					if ($lmf_var_post_values) {
						foreach ($lmf_var_post_values as $key =>  $post_data) {
							$post_id = $post_data['value']; // Extract the post ID
							list($post_id, $lmf_fields, $lmf_perks_fields) = StayEdits::defaults($post_id);
							$elevate_region_terms = get_the_terms($post_id, 'elevate-region');
							// Extract post details
							$lmf_var_post_title     = get_the_title($post_id);
							$lmf_var_post_permalink = get_permalink($post_id);
							$lmf_var_perk_description = $lmf_fields['lmf_var_perk_description'] ?? '';
							...
							<div class="swiper-slide">
								<div class="perk-slider-single">
									...
									<div class="perk-single-content">
										...
										<h2 class="heading-5">
											<a href="<?php echo esc_url(get_the_permalink($post_id)); ?>" class="no-link-style">
												<?php echo esc_html($lmf_var_post_title); ?>
											</a>
										</h2>
										<div class="perk-single-text">
											<p>
												<?php echo html_entity_decode($lmf_var_perk_description); ?>
											</p>
										</div>
